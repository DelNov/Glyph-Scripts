...
set R             0.8
set BL            [expr $R * 0.1]
set Y_FIRST_ROD   0.001
set Y_FIRST_WALL  0.004

# Concerning resolution
set N_SEG            3
set N_BND_LAY       16    
set N_CORE          13    
set N_TANG_12_TENT  26   ; # It has 12 because it covers one twelvth of the rod 
set N_TANG_12_FINE  26   ; # It has 12 because it covers one twelvth of the rod 
set N_H              5
...
set B_MAX      [expr ($W * $N_SEG) - $R * 0.5]
set W_MIN      [expr $BL * 0.5]
set W_MAX      [expr ($W * $N_SEG) - $BL * 0.5]
...
# Appended by Pointwise V17.2R2 - Tue Jan 30 11:20:32 2018
...
foreach ge [pw::Grid getAll] {
  ...
  if { [string match "dom" [string range $name 0 2]] == 1 } {
    ...
    # Walls
    if { $x_min > [expr $W*$N_SEG - $BL] } { if { $x_max > [expr $W*$N_SEG - $BL] } { 
      set block [pw::Block getBlocksFromDomains $ge]
    ...
    # More walls
    if { $y_min > [expr $W*$N_SEG - $BL] } { if { $y_max > [expr $W*$N_SEG - $BL] } { 
      set block [pw::Block getBlocksFromDomains $ge]
      lappend wall_faces [list $block $ge]
    } }
